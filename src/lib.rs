...
impl Face {
    pub fn new(first: Vec3, second: Vec3, third: Vec3) -> Face{
        let mut f = Face{verts: vec![first, second, third], normal: Vec3::new((0.0, 0.0, 1.0))};
        f.compute_normal();
        f
    }
...
impl Mesh {
    ...
    pub fn clamp(points: &Vec<Vec3>) -> Mesh {
        panic!("Not yet implemented");
    }
    ...
    pub fn generate_mesh (img: &DynamicImage, settings: &GenSettings) -> Mesh{
        ...
        for y in 0..(bounds.1) {
            for x in 0..(bounds.0) {
                ...
                let point0 = Vec3::new((coords.0, coords.1, Mesh::compute_height(img.get_pixel(x, y).channels4().0, &settings)));
                let point1 = Vec3::new((coords.0, coords.1 - 1.0, Mesh::compute_height(img.get_pixel(x, y + 1 ).channels4().0, &settings)));
                let point2 = Vec3::new((coords.0 + 1.0, coords.1, Mesh::compute_height(img.get_pixel(x + 1, y).channels4().0, &settings)));
                let point3 = Vec3::new((coords.0 + 1.0, coords.1 - 1.0, Mesh::compute_height(img.get_pixel(x + 1, y + 1).channels4().0, &settings)));
        ...
        Mesh{faces, dimensions: img.dimensions()}
    }
